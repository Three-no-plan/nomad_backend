...
async fn deploy_brc20_token(args: DeployBrc20Args) -> Result<(), String> {
    // 去 brc20 canister 验证
    let std_btc_tx = tx::decode::parse_tx_from_hash(args.deploy_tx_hash.clone()).unwrap();
...
async fn mint_brc20_token(args: MintBrc20Args) -> Result<String, String> {
    match CONTRACT_MAP.with(|map| map.borrow().get(&args.contract_id)) {
        ...
        Some(info) => {
            let user_psbt =
                Psbt::deserialize(&hex::decode(args.mint_psbt_tx_hex.clone()).unwrap()).unwrap();
            ...
            contrcat_psbt_builder
                .add_output(
                    &args.user_address,
                    user_psbt_tx.output[0].value.to_sat(),
                    None,
                )
                .unwrap();
            ...
            IDO_RECEIVE_MAP.with(|map| {
                map.borrow_mut()
                    .insert(args.token_name.clone(), IdoReceiveVec(old_vec))
            });
            CONTRACT_MAP.with(|map| {
                map.borrow_mut().insert(
                    args.contract_id,
            ...
            let combine_psbt =
                psbt::transaction::combine_psbt(&args.mint_psbt_tx_hex, &contract_psbt_hex)
                    .unwrap();
            ...
            let sig = wallet::get_schnorr_signature(
                ...
                get_derivation_path(),
                base64::decode(combine_psbt).unwrap(),
            )
            .await;
